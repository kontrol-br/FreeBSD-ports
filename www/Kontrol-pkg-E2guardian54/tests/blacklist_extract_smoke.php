...
try {
        ...
        $base = $root . '/download-preserve';
        make_tree($base);
        $valid_archive = create_archive($base, 'download-valid', array('BL/old-category/domains'));
        $cached_archive = $base . '/blacklist.tgz';
        copy($valid_archive, $cached_archive);
        $cached_hash = hash_file('sha256', $cached_archive);
        $invalid_archive = $base . '/download-invalid.tgz';
        file_put_contents($invalid_archive, "service unavailable\n");
        assert_true(!e2g_blacklist_download_archive('http://example.invalid/blacklist.tgz', $cached_archive, false, array('source_file' => $invalid_archive)), 'invalid download unexpectedly accepted');
        assert_true(hash_file('sha256', $cached_archive) === $cached_hash, 'invalid download replaced cached archive');
