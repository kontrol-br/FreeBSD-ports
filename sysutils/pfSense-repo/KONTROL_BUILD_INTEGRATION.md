...
## RELENG_2_7_2 do kontrol-br/pfsense

O build do Kontrol 2.7.2 deve exportar as três branches abaixo em
...
| --- | --- | --- |
| `Kontrol-repo.conf` | `v2_7_2`, FreeBSD 14 | Atual e padrão |
| `Kontrol-repo-previous.conf` | `v2_7_0`, FreeBSD 14 | Retorno à release anterior |
...
`Kontrol-repo.conf` de 2.7.2. O template principal não pode apontar para 2.9.0:
isso troca o repositório ativo quando o pacote `Kontrol-repo` é atualizado e
expõe o FreeBSD 14 ao `pkg` compilado para FreeBSD 16.
...
1. Publicar `Kontrol-upgrade` e o `Kontrol-repo` corrigido no repositório 2.7.2.
2. Manter `pkg-1.20.8_3` no repositório 2.7.2.
3. Publicar o repositório 2.9.0 normalmente com o `pkg` de FreeBSD 16.
4. Confirmar que a instalação de `Kontrol-repo` em 2.7.2 mantém o link ativo no
   template 2.7.2.
